...
namespace TechnologySolutions.AsciiProtocol.Sample.Services
{
        ...
        : ICommand
    {
        ...
        public ReaderCommand(Action<object> executeAction, ReaderCommandCanExecute state)
        {
            ...
            // Refresh() the command's CanExecute status as the reader state changes
            Service.Reader.StateChanged += this.Reader_StateChanged;
        }
        ...
        public bool CanExecute(object parameter)
        {
            switch (this.state)
            {
                ...
                case ReaderCommandCanExecute.WhenConnectedAndIdle:
                    return Service.Reader.IsConnected && !Service.Reader.IsInCommand;
